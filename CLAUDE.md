...
- [x] **Paso 4.1** — `backend/src/agent/investigator.py`. Recibe un candidato, consulta herramientas, y hace 2 llamadas cortas al modelo (decisión hallazgo/cerrar; redacción). `peso_amount`, `exhibits`, `entities`, `money_trail` se arman en código, nunca el modelo.
  ...
  - **Bug real encontrado y corregido (2026-09-13), en `_construir_caso` para `round_tripping`**: el validador reconcilia `peso_amount` SUMANDO POR TABLA (misma regla que el oficial) — pero el código original citaba CADA salto del ciclo como su propio exhibit de `bank_txns`, mientras `peso_amount` solo tomaba el monto del PRIMER salto. En un ciclo de 3 saltos con montos decrecientes (cada salto se queda con una comisión), la tabla `bank_txns` sumaba ~3x el `peso_amount` reportado — el validador rechazaba el hallazgo citando la aritmética rota, no una razón forense real. Confirmado en los 3 seeds con `round_tripping` que existían en este momento (501, 503, 507): en los 3, uno de los intermediarios del mismo ciclo "se salvaba" por pura suerte (su factura propia, sin relación real con el monto en disputa, resultaba casualmente la tabla más cercana a `peso_amount` dentro del 2%), mientras los demás caían con el mensaje de arreglo roto.
    - Corregido, en dos pasadas (la primera quedó incompleta, ver abajo): ahora solo UN salto del ciclo es exhibit de `bank_txns`, y `peso_amount` reconcilia exactamente (0% de diferencia) contra ese único exhibit. Los demás saltos siguen en `money_trail` (se necesitan para el diagrama y la conectividad), pero con `exhibit_id: null` — el validador solo exige que el id exista si no es `null`.
    - **Primera pasada (incompleta): se citó el PRIMER salto** (el monto que sale de la empresa). Arreglaba la reconciliación interna del hallazgo (`validate_format.py`, exhibits vs. peso_amount del propio finding) pero dejaba roto un chequeo distinto: `eval/harness.py` (`score_seed`) compara `peso_amount` contra el `peso_amount` que declaró `generate/schemes.py` para ese esquema en `truth.json` — y `plant_round_tripping` define ese número como el monto del ÚLTIMO salto (el que regresa a la empresa, DESPUÉS de la comisión que se queda cada intermediario), no el primero. Se encontró al verificar el seed 4 específicamente (el que otra sesión ya había marcado con un 2.9% de diferencia) ANTES de lanzar los seeds de reporte — con la primera pasada, `harness.score_seed()` seguía dando `peso_reconciles: False` con los mismos números exactos que esa sesión había documentado.
    - **Segunda pasada (la que quedó): se cambió a citar el ÚLTIMO salto**, no el primero. Tiene sentido más allá de solo cuadrar el número: el retorno es la prueba de que el ciclo cierra — un pago de salida solo, sin retorno, es indistinguible de pagarle a un proveedor normal. Verificado con `harness.score_seed()` en los 4 seeds con `round_tripping` disponibles (004, 501, 503, 507): `peso_reconciles: True` en los 4, exacto (`peso_claimed == peso_actual`), recall 100% y 0% falsas acusaciones en los 4. `validate_format.py --estate` PASS en los 4. Esto es lo que de verdad resuelve el pendiente que había dejado la otra sesión en la Fase 6 (línea de arriba, "seed 4 no reconcilia el peso").
    - Consecuencia esperada, no un bug nuevo: un intermediario de un ciclo de `round_tripping` que no tiene NINGÚN contrato/OC/factura propia (un vehículo puramente de paso, sin rastro documental — la firma clásica de un shell en layering) ya no acumula 3 exhibits solo con los saltos del ciclo, así que cae por debajo del mínimo y el investigador lo cierra por su cuenta (`closed_by: investigator`) con una razón específica y verídica ("Tiene 0 contratos y 0 ordenes de compra registrados..."), en vez de armar un hallazgo forzado que el validador iba a tumbar de todos modos con un mensaje sin sentido para un juez. Esto es correcto siempre que OTRO intermediario del mismo ciclo sí produzca un hallazgo válido — y en los 4 seeds probados, siempre lo hace.
    - Determinismo y formato reverificados tras el fix definitivo: `make check-isolation` en 0, `validate_format.py --estate` limpio en los 4 seeds, y una corrida en `--mode replay` reproduce el `submission.json` byte-idéntico salvo `wall_clock_seconds`.
...
- [x] **Fase 5** — `backend/src/report/case_file.py`. HTML autocontenido con las 5 secciones de `case_file_structure.md`, en orden. Lee el estate (solo para traducir RFC/CLABE/`emp_id` a nombres legibles) y `submission.json` ya producido; no corre el agente ni conoce al modelo. El money trail se dibuja como SVG generado en código (cajas + flechas, sin librerías externas). La reconciliación de pesos reutiliza `agent/validator._monto_por_tabla` — misma tolerancia del 2%, una sola fuente de verdad para esa aritmética. Incluye, casi verbatim, la limitante de esquemas entrelazados en la sección de método y límites.
  ...
  - **Nuevo (2026-09-13): `backend/src/report/results_chart.py`.** La tabla de resultados 101-110 se queda fuera de la interfaz (`frontend/`, tal como permite `FRONTEND.md`) — en su lugar, este módulo genera una imagen estática (SVG a mano, sin librerías, mismo estilo que el money trail) con el resultado agregado de una corrida de `eval/harness.py`: recall global, recall por `scheme_type`, falsas acusaciones y los tres números. `render_png()` la convierte además a PNG vía Chrome headless (`--headless --screenshot`) si hay Chrome/Chromium instalado — si no, regresa `False` sin tronar; el `.svg` solo ya sirve para la diapositiva. `case_file.py` la embebe, opcionalmente, en el resumen ejecutivo (`--results-csv`/`--results-by-type-csv`, ambos opcionales — si no se dan, la sección simplemente no aparece, nunca se inventa un resultado). Aclarado en el propio texto embebido: es la validación agregada del sistema en los seeds de reporte, no el resultado de ESE case file en particular.
    - Bug encontrado y corregido en la primera corrida: `html.escape()` tronaba al recibir un `int` (`n_seeds`) en vez de un string.
    - Bug encontrado y corregido al revisar la captura de pantalla: el SVG traía `width`/`height` fijos en píxeles (980×~590) y se salía del contenedor del case file (`max-width: 880px`) — se cortaba contra el borde derecho. Corregido a responsive (`viewBox` fijo + `style="width:100%;height:auto;max-width:980px"`), que además obligó a que `render_png()` lea el tamaño real del `viewBox` en vez de un atributo `height` que ya no existe en la etiqueta `<svg>`.
    - Verificado: capturas de pantalla vía Chrome headless del PNG suelto y del case file completo con la imagen embebida — se ve bien en ambos casos. Conteo de tags balanceado (37=37) en el case file de prueba.
  - `python -m src.report.results_chart --results out/results_report.csv --by-type out/results_report_by_type.csv --out out/results_report.svg --png out/results_report.png` genera la imagen suelta; `python -m src.report.case_file ... --results-csv ... --results-by-type-csv ...` la embebe en el case file de cualquier seed.
...
- [x] **Fase 6, seeds de tuning verificados.** `backend/eval/harness.py` corre el ciclo completo por seed (genera estate+truth si faltan, corre `cli.run()`, compara contra `truth_seedNNN.json`). Escribe `out/results_<nombre>.csv` (columnas exactas de `results_table_template.csv`, fila `TOTAL` con `llm_calls`/`mxn_cost`/`wall_clock_s` en PROMEDIO, el resto en suma) y además `out/results_<nombre>_by_type.csv` (recall por `scheme_type`, que el CSV oficial de una fila por seed no puede expresar). `parse_seeds()` acepta `"tuning"` (1-10), `"report"` (101-110), o rangos (`"1-5,8"`). Uso:
  ```
  cd backend
  python -m eval.harness --seeds tuning --out out/results_tuning.csv
  ```
  **Resultado real, ya contra el `challenger.py` corregido (668053c): recall 100% (13/13 esquemas), 0% falsas acusaciones (0/34 decoys), los 5 `scheme_type` en 100%** — `kickback` y `threshold_splitting`, que fallaban en 0% con el retador viejo, ahora perfectos. `llm_calls` promedio 6.9, `wall_clock_s` promedio 221.75 (la corrida completa tardó ~37 min con el caché del investigador tibio — el retador sí tuvo que llamar al modelo de cero en cada seed, porque su prompt cambió por completo).
  - ~~Único pendiente, no bloqueante: seed 4 no reconcilia el peso al 2%~~ — **corregido** (ver el fix de `round_tripping` bajo el Paso 4.1 de arriba, 2026-09-13): la causa era que `peso_amount` citaba el salto equivocado del ciclo (el primero, no el último — el que declara `generate/schemes.py`). Reverificado con `harness.score_seed()` sobre el seed 4 exacto: `peso_reconciles: True`, `peso_claimed == peso_actual == 1,017,790.90`.
  - **`out/` sigue sin subirse a git (`.gitignore`) — a propósito, son artefactos regenerables.** Lo único que importa preservar es el resultado (`out/results_tuning.csv` / `_by_type.csv`), no los estates/truth/submissions/caché que lo produjeron: **los estates SÍ son 100% reproducibles en cualquier máquina (mismo seed -> mismo archivo, verificado desde el Paso 2.1), pero la corrida del LLM no viaja entre máquinas** — no hay caché que compartir, así que correr el harness en otra laptop siempre vuelve a llamar al modelo de cero (más parecido a los ~82 min originales que a los ~37 min de esta corrida). Si el resultado ya corrido en una máquina es el que cuenta, lo que se lleva a otra es el CSV, no la orden de volver a correrlo.
- [x] **Fase 6 — Seeds de reporte (101-110), corrida oficial ya hecha (2026-09-13).** Antes de lanzarla se confirmó que los tres fixes pendientes (población limpia del generador, hechos del retador acotados por esquema, `round_tripping`/`peso_amount` citando el salto correcto) ya estaban integrados y verificados — condición explícita para que esta corrida contara como la de reporte, según las reglas del track (no se reporta sobre seeds afinados). Comando: `python -m eval.harness --seeds report --out out/results_report.csv`, corrida en frío (sin caché ni estates previos para estos seeds), guardada además en `out/harness_report_log.txt`.
  - **Resultado oficial: recall 100% (19/19 esquemas), 0% falsas acusaciones (0/27 decoys), `peso_reconciles: True` en los 10 seeds, los 5 `scheme_type` en 100%** (`kickback` 2/2, `phantom_vendor` 6/6, `revenue_inflation` 3/3, `round_tripping` 4/4, `threshold_splitting` 4/4). Seeds 103 y 110 no traían esquemas sembrados (solo decoys) y correctamente no acusaron a nadie. `llm_calls` promedio 9.6, `mxn_cost` $0.00, `wall_clock_s` promedio 51.47 — mucho más rápido que la estimación de ~82 min porque Ollama ya tenía el modelo cargado en memoria de pruebas previas en la misma sesión (sin costo de arranque en frío por seed).
  - Verificado además: `make check-isolation` en 0, `validate_format.py --estate` PASS en una muestra de los submissions generados (101, 105, 108).
  - **Esta es la corrida que cuenta para el reporte final — no se vuelve a correr sobre estos seeds.** Resultados en `out/results_report.csv` y `out/results_report_by_type.csv` (no viajan en git, `out/` está en `.gitignore` — si hace falta preservarlos fuera de esta máquina, copiar los CSV, no re-correr el comando).
...
- [x] **`backend/src/cli.py` ya escribe el bloque `ui`** que pedía `frontend/FRONTEND.md` (2026-09-12, hecho en `main` mientras la Fase 6 corría en otra máquina). Nuevo módulo `backend/src/report/ui_block.py` (`build(conn, empresa_rfc, findings, leads, finding_meta)`), sin modelo — puro código, reconsulta el estate solo para traducir RFC/CLABE/`emp_id` a nombres legibles (mismo patrón que `report/case_file.py`). `cli.py` acumula `finding_meta` (signal + `tool_calls_made`) por hallazgo durante el ciclo — ese dato no vivía en el finding tras el Paso 4.1, y el FRONTEND.md pide que el contraste lo tenga también del lado acusado, no solo en `leads_not_pursued`.
  ...
  - **`frontend/src/data/adaptUi.js` ya se probó contra `submission.json` reales de `cli.py` (2026-09-13)** — no con el navegador (no hay `chromium-cli` ni Playwright en este entorno, y `npm install` falla por un bug de resolución de versiones en `@esbuild/aix-ppc64`, ajeno al proyecto), sino ejecutando `adaptSubmission()` directamente en Node contra dos submissions reales (seed 4: kickback+round_tripping+threshold_splitting; seed 505: kickback+revenue_inflation) y verificando el modelo resultante a mano. Se encontraron y corrigieron dos bugs reales, los dos solo en frontend:
...
- [x] **`GraphView.jsx`: la animación del grafo ya no aparece de golpe (2026-09-13)** — antes, cada nodo/arista saltaba instantáneo de invisible a visible al cambiar `step`; ahora usa transiciones CSS puras (nada de librerías ni JS de animación):
  - Nodos: entran con `scale(0) → scale(1)` + opacidad, `transform-origin` fijado en su propio centro (`{x}px {y}px`, truco estándar para escalar un círculo SVG desde adentro). Los que comparten `step` salen con 50ms de diferencia entre sí (dentro del rango 40-60ms pedido) — `staggerDelaysFor()`, determinista por el orden ya estable de `nodes`, nunca aleatorio. El color (`fill`/`stroke`) ya transiciona en vez de saltar cuando un nodo cambia de estado (p. ej. `flagged` → `accused` en el clímax).
  - Aristas: el trazo se dibuja con el truco `pathLength="1"` + `stroke-dasharray:1` + `stroke-dashoffset` animado de 1 (oculto) a 0 (dibujado) — así no hace falta medir la geometría real de la curva. La etiqueta del monto usa `transition-delay` igual a la duración del trazo, para que aparezca solo cuando la línea ya terminó de dibujarse, nunca junto con ella.
  - El paso 6 (la revelación del destino) es más lento y marcado: nodos con `transition-duration: 900ms` y un `cubic-bezier` con leve sobresalto (vs. 600ms normal); aristas con 750ms de trazo (vs. 400ms normal). Ambos casos vía una clase `.is-climax` que solo pisa `transition-duration`/`transition-timing-function`, reutilizando el `transition-property` ya declarado en la regla base.
  - **No se tocó `useProgressiveSteps.js`** — el intervalo entre pasos (8000ms, calibrado para los ~50s de narración del pitch) sigue igual; lo que cambió es solo cuánto tarda la animación DENTRO de cada paso en asentarse, no la cadencia general de la demo.
  - Verificado sin poder correr el dev server (mismo bloqueo de `npm install`/sin navegador interactivo de antes): se generó un HTML aislado que reproduce a mano la misma lógica de `GraphView.jsx` (mismas funciones puras, mismas clases CSS, importando `app.css`/`theme.css` reales) contra datos de verdad (seed 301), renderizado con Chrome headless en dos estados — `step=3` (candidatos en ámbar/`flagged`, sin aristas, los descartados ya en verde) y `step=maxStep` (todo revelado, aristas dibujadas completas, acusados en rojo) — ambos coinciden exactamente con el comportamiento esperado. No prueba el MOVIMIENTO de la transición en sí (requeriría un navegador interactivo), pero sí que la geometría, las clases y los datos están bien conectados.
  - **Segunda pasada, ya pedida por el usuario (2026-09-13): tres refinamientos, uno de ellos descartado tras verificar.**
    1. **Descartado tras comprobar**: se sospechó un "bug de sincronía" (la flecha `marker-end` podía aparecer antes de que la línea terminara de dibujarse). Se armó una prueba aislada (un `<path>` con `marker-end` sobre un círculo de radio 18, el mismo tamaño que un nodo real) y **la flecha nunca se ve** — el retroceso del marcador (`refX=7`, escalado por `stroke-width`) la deja entera detrás del círculo del nodo, que se pinta después. No había nada que sincronizar porque el elemento ya era invisible en el diseño original. Se aprovechó para **quitar el `<marker>` y el `markerEnd` muertos** (peso sin ningún efecto visual) en vez de complicar el código arreglando algo que no se ve.
    2. **"Pop" real del clímax vía `@keyframes`** (`graph-node-pop`): en vez de solo alargar la duración de una transición, los nodos del paso 6 ahora animan `0% oculto → 55% crece de más (scale 1.18) → 75% se contrae (0.94) → 100% asienta (scale 1)`, con `animation-fill-mode: both` para que se quede oculto antes de `.is-visible` y asentado después. Para que la animación no compita con nada, estos nodos climax NO fijan `opacity`/`transform` inline (a diferencia de los demás, que siguen usando transición + estilo inline) — los controla enteramente el CSS.
    3. **Micro-interacción de hover**: un `filter: brightness(1.12)` sobre el círculo al pasar el mouse (`.graph-node-group:hover .graph-node-circle`) — se eligió `filter` a propósito porque `opacity`/`transform`/`r` ya se fijan inline por nodo (mayor especificidad que cualquier regla `:hover` en la misma propiedad), y `filter` es la única propiedad visual libre de conflicto. Nada de partículas ni movimiento — el propio `FRONTEND.md` lo prohíbe explícitamente.
    - **Verificado, con un hallazgo metodológico importante**: la primera captura del nodo clímax salió con la etiqueta rota/casi invisible — no era un bug del código, sino que Chrome headless con `--screenshot` toma la foto ANTES de que termine una animación CSS de 900ms (a diferencia de una transición disparada por cambio de estado, una `animation` empieza a correr sola en cuanto se pinta la página, y una captura estática la atrapa a medio camino). Se confirmó agregando `--virtual-time-budget=2000` (avanza el tiempo virtual antes de capturar): con eso, el nodo clímax se ve completo y correcto. Vale la pena recordar esto para cualquier verificación futura con Chrome headless sobre algo que use `animation` (no `transition`).
