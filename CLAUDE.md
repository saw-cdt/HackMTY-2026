...
- [ ] **← Empieza aquí (en `main`): seeds de reporte (101-110), todavía sin tocar.** No se usaron para tunear — corren una sola vez, ya con todo el pipeline verificado arriba: `python -m eval.harness --seeds report --out out/results_report.csv`. Sin caché ni estates previos para estos seeds (nunca se generaron), así que la corrida es en frío — cuenta con algo más cerca de los ~82 min que de los ~37 min de la corrida de tuning.
