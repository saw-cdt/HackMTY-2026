...
- [ ] **← Empieza aquí (en `main`): Fase 6** (`backend/eval/harness.py`). Corre el sistema sobre una lista de seeds, compara contra `truth_seedNNN.json`, produce la tabla de recall/falsas acusaciones/costo. El hueco de `kickback`/`challenger` mencionado arriba ya está corregido (ver Paso 4.2).
