...
- [ ] **← Empieza aquí: Paso 4.4** (`backend/src/cli.py`). Conecta el ciclo completo: detectores → investigador → retador → validador → `out/submission.json`, con `run_metadata` (llm_calls, mxn_cost, wall_clock_seconds) del envoltorio de `llm.py`. El stub actual de `cli.py` solo lanza `NotImplementedError`. Nada de `report/` ni `eval/harness.py` existe todavía.
