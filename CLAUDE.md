...
- [x] **Fase 6, seeds de tuning verificados.** `backend/eval/harness.py` corre el ciclo completo por seed (genera estate+truth si faltan, corre `cli.run()`, compara contra `truth_seedNNN.json`). Escribe `out/results_<nombre>.csv` (columnas exactas de `results_table_template.csv`, fila `TOTAL` con `llm_calls`/`mxn_cost`/`wall_clock_s` en PROMEDIO, el resto en suma) y además `out/results_<nombre>_by_type.csv` (recall por `scheme_type`, que el CSV oficial de una fila por seed no puede expresar). `parse_seeds()` acepta `"tuning"` (1-10), `"report"` (101-110), o rangos (`"1-5,8"`). Uso:
  ...
  **Resultado real, ya contra el `challenger.py` corregido (668053c): recall 100% (13/13 esquemas), 0% falsas acusaciones (0/34 decoys), los 5 `scheme_type` en 100%** — `kickback` y `threshold_splitting`, que fallaban en 0% con el retador viejo, ahora perfectos. `llm_calls` promedio 6.9, `wall_clock_s` promedio 221.75 (la corrida completa tardó ~37 min con el caché del investigador tibio — el retador sí tuvo que llamar al modelo de cero en cada seed, porque su prompt cambió por completo).
  - **Único pendiente, no bloqueante**: seed 4 no reconcilia el peso al 2% (`peso_claimed` 1,047,607.02 vs `peso_actual` 1,017,790.90, ~2.9% de diferencia — el resto de los 9 seeds sí reconcilian). Viene del `round_tripping`: el `peso_amount` que arma el investigador a partir de los exhibits del money_trail no coincide exactamente con el `peso_amount` que definió el generador para ese esquema. No afecta recall ni falsas acusaciones, pero vale la pena mirarlo (`agent/investigator.py`, construcción del `peso_amount` para `round_tripping`) antes de dar los números por definitivos.
...
- [ ] **Pendiente, y es lo que bloquea que el frontend deje de depender de datos de mentira:** `backend/src/cli.py` (Fase 4.4, ya en `main`) todavía NO escribe el bloque `ui` que pide `frontend/FRONTEND.md` (`ui.nodes`, `ui.edges`, `ui.highlight`, `ui.context`) — ese bloque sigue sin dueño en el plan de fases de arriba. Mientras tanto, `frontend/public/demo/submission_seed004.json` trae un bloque `ui` escrito a mano (no generado por el backend) para que la demo tenga contenido real, y `frontend/src/data/adaptUi.js` trae un adaptador de respaldo que infiere nodos/aristas de `findings`/`leads_not_pursued` cuando no hay `ui` — sin probar todavía contra un `submission.json` real de `cli.py`.
